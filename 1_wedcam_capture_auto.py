...
def capture_images_auto(name, base_folder=config.RAW_IMAGES_DIR, max_images=config.MAX_IMAGES_PER_PERSON, delay=config.DELAY_AUTO_CAPTURE):
...
if __name__ == "__main__":
    ...
    delay = float(input(f"Nhập thời gian delay giữa các lần chụp (giây, mặc định {config.DELAY_AUTO_CAPTURE}): ") or config.DELAY_AUTO_CAPTURE)
    capture_images_auto(name, delay=delay)